 ...
 * @category Internal
 */
export class SigaaAccountUFBA implements Account {
  /**
  ...
  private async parseStudentHomePage(homepage: Page) {
    const registration = this.parser.removeTagsHtml(
      homepage.$('#agenda-docente > table > tbody > tr:nth-child(1) > td:nth-child(2)').html()
    );
    const program = this.parser.removeTagsHtml(
      homepage.$('#info-usuario > p.periodo-atual').html()
    );
  ...
  async getProfilePictureURL(): Promise<URL | null> {
    ...
    const pictureElement = page.$('#perfil-docente > div.pessoal-docente > div.foto > img');
  ...
  async getName(): Promise<string> {
    ...
    if (page.statusCode === 200) {
      const username = this.parser.removeTagsHtml(
        page.$('#info-usuario > p.usuario > span').html()
      );
      this._name = username;
      return username;
    } else {
  ...
  async getEmails(): Promise<string[]> {
    ...
    if (page.statusCode === 200) {
      this._emails = [];
      const rows = page.$('#agenda-docente > table').eq(0).find('tr').toArray();
      for (const row of rows) {
        const cells = page.$(row).find('td');
        if (this.parser.removeTagsHtml(cells.eq(0).html()).trim().toLowerCase() == 'e-mail:') {
