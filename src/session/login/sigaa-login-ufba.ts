 ...
 * @category Internal
 */
export class SigaaLoginUFBA implements Login {
  ...
  async getLoginForm(): Promise<SigaaForm> {
    if (this.form) {
      ...
      const page = await this.http.get('/sigaa/logar.do?dispatch=logOn');
  ...
  protected async parseDesktopLoginResult(page: UFPBPage): Promise<UFPBPage> {
    ...
    if (accountPage.url.href.includes('/sigaa/logar.do')) {
